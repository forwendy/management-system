<template>
  <div>
      <div class="left">
        <span class="iconfont fold" :class="{active: !leftMenuShow}" @click="CHANGE_LEFT_MENU">&#xe66b;</span>
        <span>欢迎进入，{{ brandname }}</span>
      </div>
      <div class="right">
        <a v-button="'quick-enter'" class="quick-enter row" @click="toQuickEnter"><i class="iconfont">&#xe62a;</i>快捷入口</a>
        <span class="iconfont help">&#xe756;</span>
        <el-badge is-dot class="message"><span class="iconfont">&#xe67d;</span></el-badge>
        <span class="user">
      ...
      <!-- 修改密码 -->
      <el-dialog title="修改密码" width="80%" :visible.sync="dialogVisible" :before-close="beforeCloseDialog" append-to-body>
...
export default {
  ...
  methods:{
    ...
    exit(){
      this.$router.replace({ path: `/login/${this.brand.key}` })
    },
    set(command){
      if(command == 1){
