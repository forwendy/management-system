...
const mutations = {
  SET_MENUS_DATA(state, payload){
    state.arr = [state.home].concat(payload)
    state.tabs = [state.home]
    state.activeId = 'home'
  },
  // 添加Tab
  ...
  REMOVE_OTHER_TAB(state, id){
    const arr = state.tabs.filter(obj => obj.id == id)
    state.tabs = [state.home].concat(arr)
  },
  // 移除全部tab
  REMOVE_ALL_TAB(state){
    state.tabs = [state.home]
    state.activeId = state.home.id
    router.replace(state.home.path)
  },
  // 控制左侧菜单的显示隐藏
...
const actions = {
  ...
  setActiveTab({ commit, state }, path){
    const obj = state.tabs.find(el=>el.path == path)
    if(obj) commit('SHOW_TAB', obj.id)
  },
  // 关闭tab
